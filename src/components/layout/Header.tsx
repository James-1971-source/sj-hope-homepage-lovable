...
const navigation = [
  ...
  { 
    ...
    children: [
      { name: "공지사항", href: "/news" },
      { name: "활동소식", href: "/news?category=activity" },
      { name: "행사안내", href: "/news?category=event" },
    ]
  },
                  ...
                  <NavigationMenuItem key={item.name}>
                    {item.children ? (
                            ...
                            {item.children.map((child) => (
                              <li key={child.name}>
                                <NavigationMenuLink asChild>
                                  <Link
                                    to={child.href}
                                    className="block select-none rounded-md p-3 leading-none no-underline outline-none transition-colors hover:bg-muted hover:text-primary focus:bg-muted"
                                  >
                      ...
                      <Link
                        to={item.href}
                        className={cn(
                          "flex items-center px-4 py-2 text-sm font-medium rounded-md transition-colors hover:bg-muted hover:text-primary",
                          isActive(item.href) && "text-primary"
                        )}
                      >
                        {item.name}
                ...
                {item.children ? (
                  <div>
                    <button
                      ...
                      className={cn(
                        "flex items-center justify-between w-full px-4 py-3 text-left font-medium rounded-lg hover:bg-muted transition-colors",
                        isActive(item.href) && "text-primary"
                      )}
                    >
                      {item.name}
